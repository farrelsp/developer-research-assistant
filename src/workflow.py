...
class Workflow:
  ...
  def _analyze_company_content(self, company_name: str, content: str) -> CompanyAnalysis:
    ...
    messages = [
      SystemMessage(self.prompts.TOOL_ANALYSIS_SYSTEM),
      HumanMessage(self.prompts.tool_analysis_user(company_name=company_name, content=content))
    ]
  ...
  def _research_step(self, state: ResearchState) -> Dict[str, Any]:
    ...
    for tool_name in tool_names:
      tool_search_results = self.firecrawl.search_companies(query=tool_name + " official site",
                                                            num_results=1)
  ...
  def _analyze_step(self, state: ResearchState) -> Dict[str, Any]:
    ...
    company_data = ", ".join([company.model_dump_json() for company in state.companies])
    
    messages = [
      SystemMessage(self.prompts.RECOMMENDATIONS_SYSTEM),
      HumanMessage(self.prompts.recommendations_user(query=state.query, company_data=company_data))
    ]
